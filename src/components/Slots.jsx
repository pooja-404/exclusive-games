...
const Slots = () => {
    ...
    const card = cards.map((cards, i) => (<div key={i} className="lg:w-1/3 md:w-2/4  w-full px-3 flex justify-center group duration-500 mb-6">
        <div className='hover:bg-slot p-[1px] rounded-[8px] w-full'>
            <div className="lg:max-w-[364px] rounded-lg bg-[url(url(./assets/images/gline.webp)] w-full h-[265px] overflow-hidden relative bg-no-repeat bg-center bg-cover  bg-gradient p-[1px] " data-aos={`${cards.animation}`}    style={{ backgroundImage: `url(${cards.bg})` }} >
